...
```{r}
atlantis_box_cap$Prop[is.na(atlantis_box_cap$Prop)] <- 0

capelin_s1_s4 <- atlantis_box_cap %>% select(.bx0,Prop) %>% st_set_geometry(NULL) 

# find smallest non-zero value of the proportion
cap_min_prop <- capelin_s1_s4 %>% select(Prop) %>% filter(Prop>0) %>% pull() %>% min()
cap_max_prop <- capelin_s1_s4 %>% select(Prop) %>% filter(Prop>0) %>% pull() %>% max()

# how many boxes have a zero value?
box_no_capelin <- capelin_s1_s4 %>% filter(Prop==0) %>% pull(Prop) %>% length() # 60 boxes!

# change props
capelin_s1_s4$Prop[capelin_s1_s4$Prop==0] <- cap_min_prop
...
```{r}
write.csv(capelin_s1_s4, 'capelin_s1_s4.csv', row.names = F)
```
